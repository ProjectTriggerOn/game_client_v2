...
`local` / `remote` では ENet ホストは起動時に用意されますが、**接続はしません**。
サーバーのマッチルームに参加するのは明示的な操作（タイトルの PLAY、リザルトの
NEXT MATCH）で、ゲームを抜けると席を返します。サーバーは接続ピア数ではなく参加済み
プレイヤー数を数えるため、タイトル画面を眺めている人が枠を占有したり、マッチ開始の
最少人数に加算されたりすることはありません。参加は短いハンドシェイク（接続 → サーバーの
`MAP_INFO` と当たり判定チェックサムを照合 → `JOIN_REQUEST` → サーバーからの最初の
スナップショット）で、それが確定するまでローディングカーテンを保持するので、ゲームプレイの
最初のフレームには必ず権威的なワールドがあります。確定しないまま 5 秒経過した場合は
タイムアウトし、ハングせずに切断状態で戻ります（PLAY を押し直せば再試行）。
...
起動中の `game_server` に対して動かし、サーバー側のスイートでは検証できない側を確認
します — プロセス起動だけでは何にも参加しないこと、`BeginJoinSession` がサーバーの
スナップショット応答を受けて初めて確定すること、未消費のスナップショット滞留に上限が
あること、退出で席が返ること。先にサーバーを起動して指定してください:
