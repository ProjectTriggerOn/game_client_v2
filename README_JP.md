...
- **スケルタルアニメーション** — ASSIMP によるモデル読み込み、スナップショットベースのクロスフェード + 加算ブレンディング対応
...
- **COD 型リコイルと射撃感** — 見た目だけ跳ね上がるパンチ、実際に狙点がずれるキック、ブルームを `fireCounter` から（乱数なしで）ローカル予測し、同一内容の `recoil_math.h` を持つサーバーの値と突き合わせて補正します。クロスヘアは実際の拡散コーンに追従し、ADS 中はフェードアウト。ヒットマーカーと被弾時のビネットも備えます。
...
- **サーバーと共有する `.map` 形式** — `[network].map` が指すファイルを `resource/maps/` から読み込み（同梱の config では `shipment`）、接続時にサーバーの `MAP_INFO` の当たり判定チェックサムと実際に読み込んだマップを照合します。
...
`local` / `remote` では ENet ホストは起動時に初期化されますが、**接続はしません**。
サーバーのマッチルームに参加するのは明示的な操作（タイトルの PLAY、リザルトの
NEXT MATCH）のときだけで、ゲームを抜ければ枠を解放します。サーバーは接続ピア数ではなく
参加済みプレイヤー数を数えるため、タイトル画面にいるだけのクライアントが枠を占有したり、
マッチ開始に必要な人数に加算されたりすることはありません。参加は短いハンドシェイク
（接続 → サーバーの `MAP_INFO` と当たり判定チェックサムを照合 → `JOIN_REQUEST` →
サーバーからの最初のスナップショット）で、これが完了するまでローディング画面を出したまま
待つため、ゲームプレイの最初のフレームでは必ずサーバー権威のワールド状態が揃っています。
完了しないまま 5 秒経過した場合はタイムアウトし、ハングせずに切断状態のまま制御を返します
（PLAY を押し直せば再試行できます）。
...
| モード | 説明 | サーバー要否 |
|--------|------|-------------|
| `mock` | インプロセスのモックサーバー。オフラインでもそのまま遊べます | 不要 |
| `local` | ENet UDP で `127.0.0.1` に接続 | 要（ローカル） |
| `remote` | ENet UDP で `remote_host` に接続 | 要（リモート） |

#### `mock` — オフラインのサーバー

`mock` は HUD に数字を流し込むだけのスタブではありません。`MockServer` は権威型
サーバーのもう一つの実装で、クライアントのプロセス内に住み、ENet 側とまったく同じ
`INetwork` インターフェースの裏にいます。32 Hz のアキュムレータティックも、`InputCmd`
を受けて `Snapshot` を返す形も、`recoil_math.h` も同一です。トランスポートにあたるのが
`MockNetwork` で、ENet クライアントがソケットを持つところにミューテックス付きのキューが
2 本あるだけです。インターフェースより上はどちらと話しているかを知らないため、予測・
再シミュレート補正・補間・HUD・キルフィード・スナップショット差分からの音イベント導出が、
サーバーもネットワークもないまま本物として動きます。

シミュレートしている範囲:

- ローカルプレイヤーのサーバー側処理 — 移動、読み込んだマップとの当たり判定、重力と
  ジャンプ、RPM 制限つきのヒットスキャン射撃、リロード、残弾、死亡と 2 秒後のリスポーン
- 9 体のボット（`MAX_PLAYERS - 1`）を RED / BLUE に分けて配置。いずれも完全な戦闘
  エンティティで、往復移動し、撃たれ、バースト射撃し（600 RPM で 0.5 秒撃って 1.5 秒
  休む）、弾が尽きれば自動リロードし、死んでリスポーンします
- ボットへの射撃に対するラグ補償 — ボットごとに 64 ティック（2 秒）の位置リングを持ち、
  クライアントの `viewTick` とサブティック補間値まで巻き戻します。リスポーンのテレポートを
  跨いで補間しないガードつきです
- スコア周り一式 — K/D、チームスコア、キルフィードリング、そしてマッチを終了させて
  勝利チームを確定するスコア上限 / 制限時間
- スポーン地点は、読み込んだマップが十分な数を持っていればそこから。なければ
  コンパイル済みの配置にフォールバックします

意図的に持っていないもの:

| | `mock` | 実サーバー |
|---|---|---|
| マッチ進行 | `PLAYING` から始まります | `WAITING` → `COUNTDOWN` → `PLAYING` → `ENDED` |
| 参加 | ハンドシェイクなし。PLAY でそのまま本戦へ | 接続 → マップ照合 → `JOIN_REQUEST` → 最初のスナップショット |
| 最少人数 | なし | `MatchConfig::MIN_PLAYERS` |
| ネットワーク | プロセス内のキュー。遅延・損失・順序入れ替え・レート制限のいずれもなし | UDP と、それに伴うすべて |
| マップ照合 | 不要（同一プロセス・同一マップ） | `MAP_INFO` の当たり判定チェックサム |
| ボットの照準 | スポーン時の向きへ真っすぐ撃つだけ。交差する射線は RED / BLUE の配置から生まれます | 実際の対戦相手は狙ってきます |
| ローカルプレイヤー | 常に RED、常に id 0 | サーバーが割り当てます |

つまり `mock` はクライアントのシミュレーションと表示の層を余すところなく動かす一方、
**ネットワークの層はまったく動かしません**。遅延下での補正の挙動は `local` / `remote`
でしか確認できません。
...
| `Space` | ジャンプ |
| `左Shift`（押している間） | スプリント |
| マウス左ボタン（押している間） | 射撃（フルオート） |
| マウス右ボタン（押している間） | エイム / ADS |
| `R` | リロード |
| `E` | 武器点検（インスペクト） |
| `Tab`（押している間） | スコアボード |
| `Esc` | ポーズメニュー |
...
### レベルエディタ（`[debug].start_scene = "editor"`、Debug ビルド限定）

| 入力 | 操作 |
|------|------|
| `左Alt` + マウスドラッグ | 左＝オービット／中＝パン／右＝ドリー |
| マウスホイール | ドリー（`Alt` 不要） |
| `Q` `W` `E` `R` | 選択／移動／回転／スケール ツール |
| `B` | カーソル位置の地面に 1×1×1 のボックスブラシを配置（グリッドスナップ） |
| `N` | カタログの現在のモデルをカーソル位置に配置し、コライダーを自動生成 |
| `[` `]` | カタログの前 / 次のモデル |
| `左Ctrl`（押している間） | グリッドスナップを反転 |
| `F` | 選択物にフレーム |
| `A` | マップ全体にフレーム |
| `Delete` | 選択物を削除 |
| `Ctrl` + `Z` / `Y` | 元に戻す / やり直す |
| `F9` / `F10` | `.map` の保存 / 再読み込み |

### UI サンドボックス（`[debug].start_scene = "ui_test"`）

`F5` タイトル ／ `F6` HUD ／ `F7` ポーズ ／ `F8` 設定。

### F1 のネットコードオーバーレイ

ゲームプレイ中に **F1** を押すと診断オーバーレイが出ます。Debug ビルドだけでなく配布
ビルドでも有効です — ネットコードが実際に動いていることを README の記述ではなく目で
確かめる一番早い方法だからです。当たり判定のワイヤーフレーム（ローカルのカプセルが緑、
リモートが赤、ワールドの AABB は地面が青・壁がオレンジ、射線が黄）を、次のテキストの上に
重ねて描画します。

- **Network** — 接続状態、RTT、パケットロス、送信した入力数、未消費のスナップショット
  キュー、期待値 32/s に対する受信レート、連続するサーバーティックの間隔
- **LagComp** — クライアントが観測しているティックと、それが現在からどれだけ遅れているか
  （ティック数とミリ秒）。つまりこのクライアントの射撃に対してサーバーが当たり判定を
  どこまで巻き戻すか
- **Server** — 権威側のティック・時刻・座標。予測値との突き合わせ用
- **Correction** — 補正モード（`OK` / `SOFT` / `RESIM` / `SNAP`）と現在の位置誤差（m）
- **Input** — 直近に送信した移動軸とボタンビット
- **Player** — チーム、HP、プレイヤー / ウェポンステート、そしてローカルの発砲カウンタと
  サーバー側の値。射撃予測がずれたとき最初に現れるのがここです
- **Audio** — バックエンドの状態、再生中のボイス数、直前のスナップショット差分から導出した
  イベント数、スペーシャライザーに渡しているリスナー姿勢
- **Remotes** — リモートプレイヤー 1 人につき 1 行。チーム、補間モード
  （`INTERP` / `EXTRAP` / `WAIT` / `SNAP` / `NODATA`、停止時は `STUCK` 付き）、
  バッファ内のスナップショット数、補間係数、補間遅延
...
├── maps/                  # マップデータ（default.map, shipment.map）
...
`Game/tests/` には、エンジンに依存しないロジック（マップ I/O、デカール、パーティクル、レイキャスト、リコイル計算、音表とスナップショット差分によるイベント導出）と、接続 / 参加ハンドシェイクのスタンドアロンなテストプログラムを置いています。これらは意図的に `TriggerOn.vcxproj` に含めておらず、各ファイルが独自の `main` を持ち、VS 開発者プロンプトから `cl` で直接ビルドします。
...
`test_session_join` だけは「エンジン非依存」の例外で、実際の `ENetClientNetwork` を
起動中の `game_server` に対して動かし、サーバー側のスイートでは検証できない部分を
確認します — プロセスを起動しただけではどのマッチにも参加しないこと、
`BeginJoinSession` がサーバーからのスナップショットを受け取って初めて完了すること、
未消費のスナップショットが上限を超えて溜まらないこと、退出すれば枠が解放されること。
先にサーバーを起動して指定してください:
