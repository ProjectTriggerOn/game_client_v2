...
Edit `config/config.toml` (the `config/` folder beside the executable). Settings changed in-game are written to `config/user_settings.toml` (machine-written overlay), which takes precedence over `config.toml` and is never hand-edited.
